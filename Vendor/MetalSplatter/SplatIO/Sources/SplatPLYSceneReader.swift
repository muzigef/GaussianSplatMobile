...
private struct ElementInputMapping {
    ...
    /// Valid Graphdeco PLY `f_rest_*` scalar counts for SH0...SH3.
    /// There are three color channels and `(degree + 1)^2 - 1` non-DC
    /// coefficients per channel.
    static let supportedSphericalHarmonicsCounts: Set<Int> = [0, 9, 24, 45]
    ...
    static func elementMapping(for header: PLYHeader) throws -> ElementInputMapping {
        ...
        if let sh0_rPropertyIndex = try headerElement.index(forOptionalFloat32PropertyNamed: SplatPLYConstants.PropertyName.sh0_r),
           ...
           let sh0_bPropertyIndex = try headerElement.index(forOptionalFloat32PropertyNamed: SplatPLYConstants.PropertyName.sh0_b) {
            ...
            let prefix = SplatPLYConstants.PropertyName.sphericalHarmonicsPrefix
            let indexedRestProperties: [(suffix: Int, propertyIndex: Int)] = try headerElement.properties.enumerated().compactMap { propertyIndex, property in
                guard property.name.hasPrefix(prefix) else { return nil }
                let suffixText = property.name.dropFirst(prefix.count)
                guard let suffix = Int(suffixText), suffix >= 0 else {
                    throw SplatPLYSceneReader.Error.unsupportedFileContents(
                        "Invalid spherical harmonics property name \"\(property.name)\""
                    )
                }
                guard case .primitive(.float32) = property.type else {
                    throw SplatPLYSceneReader.Error.unsupportedFileContents(
                        "Unexpected type for property \"\(property.name)\""
                    )
                }
                return (suffix, propertyIndex)
            }
            let sortedRestProperties = indexedRestProperties.sorted { $0.suffix < $1.suffix }
            let restCount = sortedRestProperties.count
            guard supportedSphericalHarmonicsCounts.contains(restCount),
                  sortedRestProperties.map(\.suffix) == Array(0..<restCount) else {
                throw SplatPLYSceneReader.Error.unsupportedFileContents(
                    "Expected contiguous f_rest properties for SH0, SH1, SH2, or SH3; found \(restCount)"
                )
            }

            if restCount > 0 {
                let individualSphericalHarmonicsPropertyIndices = sortedRestProperties.map(\.propertyIndex)
                ...
                // Graphdeco stores coefficients channel-by-channel. The same
                // mapping works for SH1 (3/channel), SH2 (8/channel), and
                // SH3 (15/channel).
