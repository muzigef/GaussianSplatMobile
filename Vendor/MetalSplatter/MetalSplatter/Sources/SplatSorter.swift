...
class SplatSorter: @unchecked Sendable {
    ...
    init(device: MTLDevice, maximumSplatCount: Int = .max) throws {
        self.device = device
        ...
        self.state = Mutex(State(indexBuffers: indexBuffers,
                                 maximumSplatCount: max(1, maximumSplatCount)))
    }
    ...
    /// Maximum number of splats represented by sorted index buffers. Attributes
    /// for every chunk remain resident; this bounds per-frame sort and draw work.
    var maximumSplatCount: Int {
        state.withLock { $0.maximumSplatCount }
    }

    func setMaximumSplatCount(_ maximumSplatCount: Int) {
        let didChange = state.withLock { state -> Bool in
            let clamped = max(1, maximumSplatCount)
            guard state.maximumSplatCount != clamped else { return false }
            state.maximumSplatCount = clamped
    ...
    private func writeSequentialIndicesToFreeBuffer(for chunks: [ChunkReference],
        ...
        let sourceSplatCount = chunks.reduce(0) { $0 + $1.buffer.count }
        let targetSplatCount = min(sourceSplatCount, state.maximumSplatCount)
        guard targetSplatCount > 0 else { return }
        ...
        try buffer.ensureCapacity(targetSplatCount)
        buffer.count = targetSplatCount

        var writePos = 0
        forEachSampledReference(in: chunks,
                                maximumSplatCount: state.maximumSplatCount) { chunk, splatIndex in
    ...
    private func extendValidBufferWithNewChunk(_ chunk: ChunkReference,
                                               state: inout State) throws {
        // A bounded candidate set is sampled across the complete flattened
        // scene, so appending one chunk cannot be represented by appending all
        // of its points to the previous sample.
        if state.maximumSplatCount != .max {
    ...
    func updateCameraPose(position: SIMD3<Float>, forward: SIMD3<Float>) {
        let didChange = state.withLock { state -> Bool in
            let positionDelta = position - state.cameraPose.position
            let movedEnough = positionDelta.lengthSquared > 1e-6
            let rotatedEnough = !SplatRenderer.Constants.sortByDistance
                && dot(forward, state.cameraPose.forward) < 0.99999
            guard movedEnough || rotatedEnough else {
                return false
            }
    ...
    private func sortLoop() async {
        while !Task.isCancelled {
            ...
            let sortParams = state.withLock { state -> (chunks: [ChunkReference], pose: CameraPose, bufferIndex: Int, chunkGeneration: UInt64, maximumSplatCount: Int)? in
                ...
                return (state.chunks, pose, bufferIndex, state.chunkGeneration, state.maximumSplatCount)
            }
            ...
            guard let params = sortParams else {
                ...
                let shouldExit = state.withLock { state -> Bool in
                    guard !state.needsSort else { return false }
                    // Clear this flag while holding the same lock used by
                    // ensureSortLoopRunning(). A concurrent invalidation that
                    // follows can then reliably start a replacement task.
            ...
            await performSort(
                ...
                maximumSplatCount: params.maximumSplatCount
            )
        }
    ...
    private func performSort(
        ...
        let sourceSplatCount = chunks.reduce(0) { $0 + $1.buffer.count }
        let targetSplatCount = min(sourceSplatCount, maximumSplatCount)
        ...
        if sortTempStorage.count != targetSplatCount {
            sortTempStorage = Array(repeating: SplatReferenceAndDepth(chunkIndex: 0, splatIndex: 0, depth: 0), count: targetSplatCount)
        }

        // Compute depth for each splat across all chunks
        var tempIndex = 0
        if SplatRenderer.Constants.sortByDistance {
            forEachSampledReference(in: chunks, maximumSplatCount: maximumSplatCount) { chunk, i in
        ...
        do {
            try targetBuffer.ensureCapacity(targetSplatCount)
            targetBuffer.count = targetSplatCount
            for i in 0..<targetSplatCount {
    ...
    /// Visits a deterministic, uniform sample of the flattened chunk list.
    /// The mapping preserves coverage across the entire source without building
    /// a second full-scene index array.
    private func forEachSampledReference(
        in chunks: [ChunkReference],
        maximumSplatCount: Int,
        _ body: (ChunkReference, Int) -> Void
    ) {
        let sourceCount = chunks.reduce(0) { $0 + $1.buffer.count }
        let targetCount = min(sourceCount, maximumSplatCount)
        guard targetCount > 0 else { return }

        var chunkArrayIndex = 0
        var chunkGlobalStart = 0
        var chunkGlobalEnd = chunks[0].buffer.count
        for sampleIndex in 0..<targetCount {
            let globalIndex = Int(
                UInt64(sampleIndex) * UInt64(sourceCount) / UInt64(targetCount)
            )
            while globalIndex >= chunkGlobalEnd && chunkArrayIndex + 1 < chunks.count {
                chunkGlobalStart = chunkGlobalEnd
                chunkArrayIndex += 1
                chunkGlobalEnd += chunks[chunkArrayIndex].buffer.count
            }
            body(chunks[chunkArrayIndex], globalIndex - chunkGlobalStart)
        }
    }
}
