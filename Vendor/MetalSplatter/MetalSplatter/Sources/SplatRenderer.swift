...
public final class SplatRenderer: @unchecked Sendable {
    ...
    /// Upper bound for the number of splats sorted and submitted per frame.
    /// All chunk attributes remain resident so the budget can be changed without
    /// reloading the scene.
    public var maximumRenderedSplatCount: Int {
        sorter.maximumSplatCount
    }

    public func setMaximumRenderedSplatCount(_ count: Int) {
        sorter.setMaximumSplatCount(count)
    }
    ...
    public init(device: MTLDevice,
                ...
                maximumRenderedSplatCount: Int = .max,
        ...
        self.sorter = try SplatSorter(device: device,
                                      maximumSplatCount: maximumRenderedSplatCount)
    ...
    /// Adds multiple pre-built chunks with one renderer/sorter synchronization.
    /// This avoids repeatedly rebuilding or patching a scene-wide index buffer
    /// while a large PLY is progressively encoded.
    @discardableResult
    public func addChunks(_ newChunks: [SplatChunk],
                          sortByLocality: Bool = true,
                          enabled: Bool = true) async -> [ChunkID] {
        ...
        return await withChunkAccess {
            var addedIDs: [ChunkID] = []
            await sorter.withExclusiveAccess {
                let availableCount = max(0, Constants.maxChunks - orderedChunkIDs.count)
                let acceptedChunks = newChunks.prefix(availableCount)
                if acceptedChunks.count != newChunks.count {
                    Self.log.warning("Maximum chunk count (\(Constants.maxChunks)) exceeded; trailing chunks were not added")
                }

                var ids: [ChunkID] = []
                ids.reserveCapacity(acceptedChunks.count)
                for chunk in acceptedChunks {
                    let id = ChunkID(rawValue: nextChunkID)
                    nextChunkID += 1
                    chunks[id] = ChunkEntry(id: id, chunk: chunk, isEnabled: enabled)
                    orderedChunkIDs.append(id)
                    ids.append(id)
                }

                chunkIDToIndex.removeAll(keepingCapacity: true)
                let references: [SplatSorter.ChunkReference] = orderedChunkIDs.enumerated().compactMap { index, id in
                    guard let entry = chunks[id] else { return nil }
                    let chunkIndex = UInt16(index)
                    chunkIDToIndex[id] = chunkIndex
                    return SplatSorter.ChunkReference(chunkIndex: chunkIndex,
                                                      buffer: entry.chunk.splats)
                }
                sorter.setChunks(references)
                addedIDs = ids
            }
            return addedIDs
        }
