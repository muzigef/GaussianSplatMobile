...
public struct SplatChunk: @unchecked Sendable {
    public enum Error: LocalizedError {
        case mixedSphericalHarmonics(expected: Int, actual: Int, pointIndex: Int)

        public var errorDescription: String? {
            switch self {
            case let .mixedSphericalHarmonics(expected, actual, pointIndex):
                return "All splats in a chunk must have the same SH coefficient count; point \(pointIndex) has \(actual), expected \(expected)"
            }
        }
    }
    ...
    public init(device: MTLDevice,
        ...
        let expectedCoefficientCount = shDegree.coefficientCount
        for (pointIndex, point) in points.enumerated() {
            let actualCoefficientCount = point.color.asSphericalHarmonicFloat.count
            guard actualCoefficientCount == expectedCoefficientCount else {
                throw Error.mixedSphericalHarmonics(
                    expected: expectedCoefficientCount,
                    actual: actualCoefficientCount,
        ...
        if shDegree > .sh0 {
            ...
            for (i, point) in points.enumerated() {
                ...
                let coefficients = point.color.asSphericalHarmonicFloat
                var destination = offset
                for coefficient in coefficients.dropFirst() {
                    shBuffer.values[destination] = Float16(coefficient.x)
                    shBuffer.values[destination + 1] = Float16(coefficient.y)
                    shBuffer.values[destination + 2] = Float16(coefficient.z)
                    destination += 3
                }
            }
